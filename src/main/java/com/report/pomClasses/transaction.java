...
public class transaction {
	...
	@FindBy(xpath = "//input[@placeholder='Search Merchant']")
	...
	@FindBy(xpath="(//button[@title='Last Action'])[1]")private WebElement lastAction;
	@FindBy(xpath="//span[@id='lAhistory']")private WebElement transactionID;
	@FindBy(xpath="(//table[@class='table table-bordered'])[5]/descendant::th")private WebElement appProperties;
	@FindBy(xpath="//h4[text()='Last Action']/following-sibling::button")private WebElement cancelButtonOnLastAction;
	
	//History button in Transaction Table
	@FindBy(xpath="(//button[@title='History'])[1]")private WebElement history;
	@FindBy(xpath="//h4[text()='Status History']") private WebElement statusHistory;
	@FindBy(xpath="//span[@id='phistory']")private WebElement historyTransaction;
	@FindBy(xpath="//h4[text()='Status History']/following-sibling::button")private WebElement cancelButtonOnHistory;
	
	//Remarks button in Transaction table
	
	@FindBy(xpath="(//button[@title='Remarks'])[1]")private WebElement remarks;
	@FindBy(xpath="//input[@id='Transaction_Id']")private WebElement remarksTransactionId;
	@FindBy(xpath="(//button[@class='btn btn-primary'])[2]")private WebElement sendMessageButton;
	@FindBy(xpath="//div[text()='All input fields must be completed before submitting.']")private WebElement errorMessage;
	@FindBy(xpath="(//h5[@id='exampleModalLabel'])[2]/following-sibling::button")private WebElement canCel_Remarks;
	@FindBy(xpath = "(//label[text()='Message'])[2]")private WebElement message;
	@FindBy(xpath="//button[text()='OK']")private WebElement OK;
	
	@FindBy(xpath="//select[@id='usertype-select']")private WebElement userType;
	@FindBy(xpath="(//span[@aria-disabled='false'])[2]")private WebElement selectUserType;
	@FindBy(xpath="//select[@id='recipient-select']")private WebElement users;
	
	@FindBy(xpath="//select[@id='recipient-Error']")private WebElement selectError;
	
	@FindBy(xpath="//textarea[@id='message-text']")private WebElement error_Message;
	@FindBy(xpath="//div[text()='Message sent successfully']")private WebElement successsMessage;

	// variable directory
	String partialMerchantname = "Rohit";
	String merchant = "RohitM";
	 ...
	 * Custom Range
	 */
	final static String dateRange = "Custom Range";
...
//  select - time zone
	...
	public void filterTransactionThroughSelectStatus() {
		...
		utility.selectByVisibleText(selectStatus, status1);
	...
	public void selectDateRange(WebDriver driver) throws InterruptedException {
		...
		for (WebElement s : suggestions) {
			if (dateRange.equalsIgnoreCase(s.getText().trim())) {
				s.click();
				Reporter.log("select date range :- " + dateRange, true);
				break;
...
//				a.click(dateTo).build().perform();
	...
	public void selectCurrency(WebDriver driver) throws InterruptedException {
		...
		WebElement tm2 = w
				.until(ExpectedConditions.presenceOfElementLocated(By.xpath("(//td[text()='USD'])[1]")));
	 ...
	 public void testSelectFilters(WebDriver driver) throws InterruptedException {
		 WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));
	        ...
	        for (int i = 6; i < totalOptions; i++) { // Start from 1 to skip "Select" or default
	           ...
	           // wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath("(//a[contains(@class,'colorpid lstStatusBtn')])[1]")));
	            ...
	            for (int rowIndex = 0; rowIndex < rows.size(); rowIndex++) {
	                WebElement statusCell = rows.get(rowIndex).findElement(By.xpath("(//a[contains(@class,'colorpid lstStatusBtn')])[1]")); // Update column index if needed
	                String actualStatus = statusCell.getText().trim();

	                Reporter.log("Row " + (rowIndex + 1) + ": Expected = " + filterValue + ", Found = " + actualStatus, true);
	                Assert.assertTrue(actualStatus.equalsIgnoreCase(filterValue),
	                	    "❌ Status mismatch at row " + (rowIndex + 1) + ": Expected = " + filterValue + ", Found = " + actualStatus);
	            }
		  ...
		  public void lastActionOnTransactionTable(WebDriver driver) throws InterruptedException {
			  ...
			  Thread.sleep(2000);
			 List<WebElement> appPropertys = w.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//tbody[@id='lastActionList']/tr")));
		  ...
		  public void historyOnTransaction(WebDriver driver) {
			...
			//  List<String> expectedValues = Arrays.asList("DATE", "STATUS", "UPDATED BY");
				...
				//List<WebElement> actualValues = w.until(
					//	ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//th[@class='text-center tdwith11']")));
			  
			  
				// Assert.assertEquals(actualValues, expectedValues, "Mismatch in element list values");
...
//        }
        ...
        String[] expectedArray = {"Select a Users","ADMIN","FULLADMIN","SUPERADMIN", "REPORTADMIN","ACCOUNTADMIN","MERCHANT","AGENT"};
...
//	        }
	        ...
	        // Using array and converting to list
	        String[] expectedArray = {"Select a Error", "Invalid Phone Number", "Bank Decline", "Blocked Payment","Insufficient Funds","Expired card","Payment gateway errors","Security concerns","Unknown errors"};
	        List<String> expectedValues = Arrays.asList(expectedArray);
	        ...
	        public void verifySuccessmessage(WebDriver driver) {
	        	WebDriverWait w = new WebDriverWait(driver, Duration.ofSeconds(30)); 
	        	 WebElement messageElement = w.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[text()='Message sent successfully']")));
